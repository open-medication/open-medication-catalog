...
Pack GTIN is emitted as `https://www.gs1.org/gtin`. ATC codes use `http://www.whocc.no/atc` (including ATCvet). `rodzajPreparatu` and target species (`nazwaGatunku`) are identifiers on the product (`pl-rpl-preparation-type`, `pl-rpl-species`). Packs also carry `rodzajPreparatu` so R4 `Medication` (and R5 PPD) can tell human from veterinary. Dose form, route, and regulatory status stay RPL source text (no invented EDQM). Pack `regulatoryStatus` is withdrawn (`skasowane`) vs active (`aktywne`) — that is the only RPL cancelled flag. Product and authorization status are `aktywne` while the row is in this dump (RPL has no product/MA `skasowane`). A product that disappears in a later month shows up in `changes.json`. MA validity (`waznoscPozwolenia`) stays in metadata — it is a date or `Bezterminowe`, not a status code. Rx/OTC (`kategoriaDostepnosci`) stays in metadata.
