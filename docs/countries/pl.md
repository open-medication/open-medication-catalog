...
| Domain | `rodzajPreparatu` (`ludzki` / `weterynaryjny`) | Medication extension `domain` | `MedicinalProductDefinition.domain` |
...
Pack GTIN is emitted as `https://www.gs1.org/gtin`. ATC codes use `http://www.whocc.no/atc` (including ATCvet codes such as `QI09AL01`). `rodzajPreparatu` stays as source provenance (`metadata.rodzajPreparatu` and a MedicinalProduct identifier `pl-rpl-preparation-type`); it is **not** copied onto Package identifiers. Human vs veterinary on FHIR is `domain`, not that identifier. Target species (`nazwaGatunku`) are identifiers on the product (`pl-rpl-species`). Dose form, route, and regulatory status stay RPL source text (no invented EDQM). Pack `regulatoryStatus` is withdrawn (`skasowane`) vs active (`aktywne`) — that is the only RPL cancelled flag. Product and authorization status are `aktywne` while the row is in this dump (RPL has no product/MA `skasowane`). A product that disappears in a later month shows up in `changes.json`. MA validity (`waznoscPozwolenia`) stays in metadata — it is a date or `Bezterminowe`, not a status code. Rx/OTC (`kategoriaDostepnosci`) stays in metadata.
