...
5. Add a recipe in `artifacts.yaml` (`fr-base`, `ch-vet-base`, etc.). Official CLI is `omc build <recipe-id>`. Set `domain: Human` or `Veterinary` when the source splits that way.
